...
def submit():
    ...
    x=E.get()
    y=F.get()
    mycon=mysql.connector.connect(host="localhost",password="kiki08",user="root",database="forms")
    mycur=mycon.cursor()
    mycur.execute("select concat(FIRSTNAME,'_',LASTNAME) AS USER,PASSWORD from form")
    data=mycur.fetchall()
    for i in data:
        if(i[0]==x):
            if(i[1]==y):
                ...
                def roll_a_die():
                    ...
                    f=open(r"C:\Users\dell\OneDrive - Amity University\Desktop\store.txt","r")
                    r=csv.reader(f)
                    for e in r:
                        if(e[0]==x):
                ...
                def play_a_riddle():
                    f=open(r"C:\Users\dell\OneDrive - Amity University\Desktop\questions.txt","r")
                    l=f.readlines()
                ...
                def score():
                    ...
                    i=y[1]
                    g=t*10
                    con=mysql.connector.connect(user="root",password="kiki08",host="localhost",database="project")
                    cur=con.cursor()
                    if(g<30):
...
def register():
    def display():
        con=mysql.connector.connect(host="localhost",password="kiki08",user="root",database="forms")
        cur=con.cursor()
        x=d.get()
        y=e.get()
        z=f.get()
